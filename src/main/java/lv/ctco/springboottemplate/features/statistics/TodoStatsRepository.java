...
public class TodoStatsRepository {
    ...
    Aggregation getSummaryStatsAggregation(ResponseFormat format, Instant from, Instant to) {
        ...
        // --- First facet: counts ---
        FacetOperation countsFacet = Aggregation.facet(
                        Aggregation.group()
                                .count().as("totalTodos")
                                .sum(ConditionalOperators.when(Criteria.where("completed").is(true)).then(1).otherwise(0))
                                .as("completedTodos")
                                .sum(ConditionalOperators.when(Criteria.where("completed").is(false)).then(1).otherwise(0))
                                .as("pendingTodos")
                ).as("counts")

                // --- Second facet: userStats ---
                .and(
                        Aggregation.group("createdBy")
                                .count().as("count")
                ).as("userStats");
        ...
        ProjectionOperation project = Aggregation.project()
                ...
                .as("pendingTodos")
                .and("userStats").as("userStats");

        if (format == ResponseFormat.DETAILED) {
            project = project.and(
                            ConditionalOperators.ifNull("completedTodos").then(List.of())
                    ).as("todos.completed")
                    .and(
                            ConditionalOperators.ifNull("pendingTodos").then(List.of())
                    ).as("todos.pending");
        }
    ...
    public TodoStatsDto getStats(ResponseFormat format, Instant from, Instant to) {
        ...
        Document doc = mongoTemplate.aggregate(aggregation, "todos", Document.class)
        ...
        Map<String, Long> userStats = userStatsList.stream()
        ...
        if (format == ResponseFormat.DETAILED) {
            List<CompletedTodoDto> completedTodos = ((List<Document>)
                    ((Document) doc.get("todos"))
                            .get("completed"))
                    .stream()
                    .map(d -> new CompletedTodoDto(
                            d.getString("id"),
                            d.getString("title"),
            ...
            List<PendingTodoDto> pendingTodos = ((List<Document>)
                    ((Document) doc.get("todos"))
                            .get("pending"))
                    .stream()
                    .map(d -> new PendingTodoDto(
                            d.getString("id"),
                            d.getString("title"),
                            d.getString("createdBy"),
                            d.getDate("createdAt").toInstant()
